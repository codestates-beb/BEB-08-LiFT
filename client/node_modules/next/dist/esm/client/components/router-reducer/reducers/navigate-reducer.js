...
export function navigateReducer(state, action) {
    ...
    if (prefetchValues) {
        ...
        for (const flightDataPath of flightData){
            ...
            if (newTree !== null) {
                ...
                const applied = applyFlightData(currentCache, cache, flightDataPath, true);
    ...
    if (forceOptimisticNavigation) {
        ...
        segments.slice(1), ()=>fetchServerResponse(url, optimisticTree, state.nextUrl));
    ...
    if (!cache.data) {
        cache.data = createRecordFromThenable(fetchServerResponse(url, state.tree, state.nextUrl));
    }
    // Unwrap cache data with `use` to suspend here (in the reducer) until the fetch resolves.
