...
async function getBaseWebpackConfig(dir, { buildId , config , compilerType , dev =false , entrypoints , isDevFallback =false , pagesDir , reactProductionProfiling =false , rewrites , originalRewrites , originalRedirects , runWebpackSpan , appDir , middlewareMatchers , noMangling =false , jsConfig , resolvedBaseUrl , supportedBrowsers , clientRouterFilters  }) {
    ...
    if (isClient) {
        ...
        if (useExperimentalReact && !hasAppDir) {
            Log.warn("You are using `experimental.experimentalReact` which requires `experimental.appDir` to be enabled.");
        }
    }
                        ...
                        oneOf: [
                            ...
                            },
                            {
                                ...
                                resolve: {
                                    alias: {
                                        react: `next/dist/compiled/react${builtInReactChannel}`,
                                        "react-dom$": reactProductionProfiling ? `next/dist/compiled/react-dom${builtInReactChannel}/cjs/react-dom.profiling.min` : `next/dist/compiled/react-dom${builtInReactChannel}`,
                                        "react-dom/client$": `next/dist/compiled/react-dom${builtInReactChannel}/client`
                                    }
                                }
                            }, 
    ...
    const cache = {
        ...
        version: `${"13.3.1-canary.6"}|${configVars}`,
